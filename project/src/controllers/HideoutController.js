...
class HideoutController
{
    ...
    static scavCaseProductionStart(pmcData, body, sessionID)
    {
        ...
        for (const rarity in recipe.EndProducts)
        {
            ...
            {
                rarityItemCounter[rarity] = RandomUtil.getInt(recipe.EndProducts[rarity].min, recipe.EndProducts[rarity].max);
            }
        }
        ...
        for (const rarityType in rarityItemCounter)
        {
            ...
            for (let idx = 0; idx < Object.keys(DatabaseServer.tables.templates.clientItems.data).length; idx++)
            {
                randomIdx.push(Object.keys(DatabaseServer.tables.templates.clientItems.data)[idx]);
            }
            ...
            for (let random = 0; random < randomIdx.length && rarityItemCounter[rarityType] > 0; random++)
            {
                const tempItem = DatabaseServer.tables.templates.clientItems.data[randomIdx[random]];
                if (tempItem === null)
                    continue;
                const isItemInBlacklist = HideoutConfig.scavCase.rewardItemBlacklist.includes(tempItem._id) || HideoutConfig.scavCase.rewardParentBlacklist.includes(tempItem._parent);

                if (tempItem._props && tempItem._props.Rarity === rarityType && !isItemInBlacklist)
                {
                    ...
                    if (tempItem._parent === ItemHelper.BASECLASS.Ammo || tempItem._parent === ItemHelper.BASECLASS.Money)
                    {
                        var amount = HideoutController.getRandomAmountRewardForScavCase(tempItem);
                        newItem.upd = {
    ...
    static getRandomAmountRewardForScavCase(itemToCalculate)
    {
        ...
        if (itemToCalculate._parent === ItemHelper.BASECLASS.Ammo)
        {
            ...
            {
                const maxMultiplier = Math.floor(itemToCalculate._props.StackMaxSize / 10);
                let minMultiplier = Math.floor(HideoutConfig.scavCase.ammoRewards.minAmount / 10);
                if (minMultiplier > maxMultiplier)
                {
                    minMultiplier = maxMultiplier;
                }
                amountToGive = RandomUtil.getInt(minMultiplier, maxMultiplier) * 10;
            }
            else
            {
                ...
                amountToGive = RandomUtil.getInt(HideoutConfig.scavCase.ammoRewards.minAmount, itemToCalculate._props.StackMaxSize);
            }
        }
        else if (itemToCalculate._parent === ItemHelper.BASECLASS.Money)
        {
            switch (itemToCalculate._id)
            {
                case ItemHelper.MONEY.Roubles:
                    amountToGive = RandomUtil.getInt(HideoutConfig.scavCase.moneyRewards.rub.min, HideoutConfig.scavCase.moneyRewards.rub.max);
                    break;
                case ItemHelper.MONEY.Euros:
                    amountToGive = RandomUtil.getInt(HideoutConfig.scavCase.moneyRewards.eur.min, HideoutConfig.scavCase.moneyRewards.eur.max);
                    break;
                case ItemHelper.MONEY.Dollars:
                    amountToGive = RandomUtil.getInt(HideoutConfig.scavCase.moneyRewards.usd.min, HideoutConfig.scavCase.moneyRewards.usd.max);
                    break;
            }
        }
        return amountToGive;
    ...
    static takeProduction(pmcData, body, sessionID)
    {
        ...
        if (recipe)
        {
            ...
            const itemsToAdd = pmcData.Hideout.Production[prod].Products.map(x =>
            {
                ...
                const numOfItems = !x.upd || !x.upd.StackObjectsCount ? 1 : x.upd.StackObjectsCount;
    ...
    static updatePlayerHideout(sessionID)
    {
        ...
        for (const prod in pmcData.Hideout.Production)
        {
            ...
            {
                pmcData.Hideout.Production[prod] = HideoutController.updateBitcoinFarm(pmcData, btcFarmCGs, isGeneratorOn);
                continue;
            }
    ...
    static updateFuel(generatorArea, pmcData)
    {
        ...
        const hideoutManagementConsumptionBonus = 1.0 - HideoutController.getHideoutManagementConsumptionBonus(pmcData);
    ...
    {
        const btcProd = pmcData.Hideout.Production[BITCOIN_FARM];
        ...
        const final_prodtime = 145000 / (1 + (btcFarmCGs - 1) * 0.041225);
    ...
    {
        const bitcoinProduction = DatabaseServer.tables.hideout.production.filter(p => p._id === BITCOIN_FARM);
        const productionSlots = bitcoinProduction?.productionLimitCount || 3;
        const hasManagementSkillSlots = HideoutController.hasManagementSkillSlots(pmcData);
        const managementSlots = HideoutController.getManagementSkillsSlots() || 2;
        return productionSlots + (hasManagementSkillSlots ? managementSlots : 0);
    }

    static getManagementSkillsSlots()
    {
        return DatabaseServer.tables.globals.config.SkillsSettings.HideoutManagement.EliteSlots.BitcoinFarm.Container;
    }

    static hasManagementSkillSlots(pmcData)
    {
        return HideoutController.getHideoutManagementSkill(pmcData)?.Progress >= 5100;
    }
    ...
    {
        const hideoutManagementSkill = HideoutController.getHideoutManagementSkill(pmcData);
        ...
        return (roundedLevel * DatabaseServer.tables.globals.config.SkillsSettings.HideoutManagement.ConsumptionReductionPerLevel) / 100;
    }
}
